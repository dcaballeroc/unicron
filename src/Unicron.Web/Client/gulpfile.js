...
gulp.task('build-specs-html', ['serve-dev', 'fonts', 'images'], function () {
    ...
    return gulp
        ...
        .pipe(plugins.inject(gulp.src(config.testlibraries), { name: 'inject:testlibraries', read: false }))
        .pipe(plugins.inject(gulp.src(config.compiledJs)))
        .pipe(plugins.inject(gulp.src(config.specHelpers), { name: 'inject:spechelpers', read: false }))
        .pipe(plugins.inject(gulp.src(config.compiledSpecs), { name: 'inject:specs', read: false }))
        .pipe(plugins.inject(gulp.src(config.temp + config.templateCache.file), { name: 'inject:templates', read: false }))
...
function compile_ts_with_maps(source, dest, isSpecs) {
    ...
    var stream = tsResults
        .pipe(plugins.sourcemaps.write(path, configMaps))
        .pipe(gulp.dest(dest));
    return stream;
}
